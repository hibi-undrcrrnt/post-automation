...
| Apps Scriptデプロイ | 標準GCP接続・E2E確認済み、行8限定`canary`稼働中 |
| Instagram実投稿テスト | 行8の画像カナリア成功、動画は未実施 |
...
すでにサービスアカウントへ共有済みのため、投稿2時間前からの事前変換を
確認したうえで予定時刻の既存投稿処理へ渡す。

将来日時のStories動画行は現時点で存在しない。行5は過去日時かつ
`error`の終端状態なので、自動再試行しない。動画カナリアは新しい将来行を
用意し、投稿を伴わない事前変換が`ready`になったことを確認してから行う。
