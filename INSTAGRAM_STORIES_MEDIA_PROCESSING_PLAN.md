...
| Apps Scriptデプロイ | 標準GCP接続・E2E確認済み、`enforce`で本番稼働中 |
| Instagram実投稿テスト | 行8画像・行9動画のカナリア成功 |
...
2026-07-25 15:02 JSTに行8限定`canary`を有効化し、
`prepareStories`の10分間隔トリガーを作成した。
同日15:12 JSTに行8の画像変換が正常完了し、
15:13 JSTにInstagram Storiesへの画像カナリア投稿が
`published`へ到達した。実機確認でも、横長画像全体が表示され、
中央クロップされていないことを確認済み。

同日15:47 JSTに行9の横長動画カナリアも`published`へ到達し、
実機確認で動画全体が正しく表示されることを確認済み。

同日17:49 JSTに`enableStoriesTransformForHibi()`を実行し、
`STORY_TRANSFORM_MODE=enforce`へ切り替えた。これ以降、
全Instagram Stories画像・動画を全体表示変換の対象として本番運用する。
既存の`prepareStories` 10分間隔トリガーはそのまま継続する。
...
5. 本番で動画1件をカナリア投稿（完了）
6. 画像・動画カナリアの実機表示とエラーログを確認（完了）
7. 運用者判断で全Stories投稿へ有効化（完了）

画像カナリアには行8、動画カナリアには行9を使用した。
いずれも元の終端行を再試行せず、Storiesだけが有効なテスト行として
実行したため、XやInstagram通常投稿の重複は発生していない。

行5は過去日時かつ`error`の終端状態なので、自動再試行しない。
行5と同じ横長動画は行9の動画カナリアで正常投稿できたため、
行5は従来処理で失敗した監査履歴として残す。

異常時は`pauseStoriesTransformForHibi()`でStories処理を停止し、
必要なら`deleteStoriesTransformTrigger()`で事前変換トリガーを削除する。
`useLegacyStoriesTransformForHibi()`は従来処理へ戻す操作であり、
横長素材が再び中央クロップされ得るため、緊急時にも意図を確認して使う。

## 11. 受け入れ条件

- 横長画像の全体が表示される（画像カナリアで確認済み）
- 横長動画の全体が表示される（動画カナリアで確認済み）
- 余白が上下または左右へ均等に入る
- 9:16素材が不要に劣化しない
- タイムアウト後に変換を再開できる
- 同じ行を複数回実行しても二重投稿しない
